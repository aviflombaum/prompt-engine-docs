...
PromptEngine offers flexible configuration options through Rails credentials, initializers, and environment variables. This guide covers all configuration options to customize PromptEngine for your needs.

## API Provider Credentials

<Callout type="warning">
  API credentials are required for the playground feature. Without them, you can still manage prompts but won't be able to test them.
</Callout>
...
<Tabs items={['Rails Credentials', 'Environment Variables']}>
  <Tab value="Rails Credentials">
    The recommended approach is using Rails encrypted credentials:
    ...
    openai:
      ...
      organization_id: org-optional-org-id  # Optional
    ...
    <Callout type="info">
      You only need to configure the providers you plan to use. Having both configured gives you more flexibility in the playground.
    </Callout>
  </Tab>
  ...
  <Tab value="Environment Variables">
    ...
    Add these to your `.env` file in development or set them in your production environment.
  </Tab>
</Tabs>
...
<TypeTable>
  <TypeTableRow 
    name="PROMPT_ENGINE_API_TIMEOUT" 
    type="integer" 
    default="30" 
    description="Timeout for API requests in seconds" 
  />
  <TypeTableRow 
    name="PROMPT_ENGINE_MAX_TOKENS" 
    type="integer" 
    default="4000" 
    description="Maximum tokens per API request" 
  />
  <TypeTableRow 
    name="PROMPT_ENGINE_LOG_LEVEL" 
    type="string" 
    default="info" 
    description="Logging level (debug, info, warn, error)" 
  />
  <TypeTableRow 
    name="PROMPT_ENGINE_ASYNC_JOBS" 
    type="boolean" 
    default="false" 
